...
function Footer() {
  ...
  const icons = [
    ...
    { icon: <FaPinterest size={30}  color='#111827' />, link: "https://www.pinterest.com/golden_designs07/" },
    { icon: <FaYoutube size={30} color='#111827' />, link: "https://www.youtube.com/channel/UC4L2PqObzbKbkk7VNiMsulw?sub_confirmation=1" },
  ];
  return (
    <footer className='bg-gray-900 p-5 md:p-12 text-white'>
        ...
        <div>
          <div className='flex-col md:flex-row flex md:justify-between md:items-center items-center'>
            ...
            <div className='flex gap-4 md:mt-0 mt-5'>
              {icons.map((icon, i) => (
                <Link key={i} href={icon.link} target='_blank' className='w-7 h-7 p-1 md:w-10 md:h-10 md:p-2 bg-white rounded-full flex items-center justify-center'>
                  {icon.icon}
                </Link>
              ))}
            </div>
